...
const vitigni = [
  {
    ...
    image: "/foto/sito/close-up-17-scaled.webp",
  ...
  },
  {
    ...
    image: "/foto/sito/close-up-64-scaled.webp",
    characteristics: [
      "Colore rosso-rosa",
      "Grappolo spargolo",
      "Buccia sottile",
  ...
  },
  {
    ...
    description:
      "Selezione esclusiva di Tenuta Belvedere. Grappoli con buccia spessa e ricca di antociani per uve di grande struttura.",
    image: "/foto/sito/close-up-9-scaled.webp",
    ...
    <section ref={sectionRef} id="vini" className="relative">
      ...
      {vitigni.map((vitigno, index) => {
        ...
        return (
          <div
            key={vitigno.id}
            className={`relative min-h-screen ${bgColor} overflow-hidden`}
          >
            <div className="grid grid-cols-1 lg:grid-cols-2 min-h-screen">
              {/* Image - alternates left/right */}
              <div
                className={`relative h-[50vh] lg:h-auto lg:min-h-screen overflow-hidden ${isEven ? "lg:order-1" : "lg:order-2"}`}
              >
                <img
                  src={vitigno.image}
                  alt={vitigno.name}
                ...
                />
                <div
                  className={`absolute inset-0 ${isEven ? "bg-gradient-to-r from-transparent to-white/20 lg:bg-gradient-to-l lg:from-white/20 lg:to-transparent" : "bg-gradient-to-l from-transparent to-chiarli-stone/20 lg:bg-gradient-to-r lg:from-chiarli-stone/20 lg:to-transparent"}`}
                />
              </div>

              {/* Content - alternates right/left */}
              <div
                className={`flex items-center py-16 md:py-24 lg:py-0 ${isEven ? "lg:order-2" : "lg:order-1"}`}
              >
